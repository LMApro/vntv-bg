...
objPage.init = function(){
    iUnit = 10;
    $('.preloader').toggle();
    // load danh sach ma chung khoan hnx
    if(dsHNXIndex == ''){
        $.ajaxSetup({ "async": false });
        $.getJSON(getListCKIndex + 'hnx',function(rData){
            // console.log(rData);
            dsHNXIndex = rData.join(',');									
            initWebsocket();
        },
        "jsonp"
        );
...
function socketConnect(){
    var msg = "{\"action\":\"join\",\"list\":\""+ dsHNXIndex +"\"}";
    socket.emit('regs', msg);
...
function initListStock() {
    ...
    vStockListSelected = dsHNXIndex;
    // console.log(dsHNXIndex);
    var abc = dsHNXIndex.split(',');
    // $.ajaxSetup({ "async": false });
