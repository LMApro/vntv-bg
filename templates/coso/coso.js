...
objPage.init = function(){
    iUnit = 10;
    if(objPage.hasOwnProperty('shareCode')){
        $('#share-modal').modal();
    } else {
        loadLocalStoreage();
        initWebsocket();
...
function socketConnect(){
    ...
    var msg = "{\"action\":\"join\",\"list\":\""+ stockHandle.symbols.join(',') +"\"}";
    socket.emit('regs', msg);
...
function initListStock() {
    ...
    // console.log(vStockListSelected);
    $.getJSON(listStockDetailLink + vStockListSelected,{},function(rData){
        if (rData != null){				
            rData.forEach(element => {
