...
objPage.init = function(){
    iUnit = 10;
    $('.preloader').toggle();
    // load danh sach ma chung khoan upcom
    if(dsUpcomIndex == ''){
        $.ajaxSetup({ "async": false });
        $.getJSON(getListCKIndex + 'upcom',function(rData){
            dsUpcomIndex = rData.join(',');								
            initWebsocket();
        },
        "jsonp"
        );
...
function socketConnect(){
    var msg = "{\"action\":\"join\",\"list\":\""+ dsUpcomIndex +"\"}";
    socket.emit('regs', msg);
...
function initListStock() {
    ...
    vStockListSelected = dsUpcomIndex;

    var abc = dsUpcomIndex.split(',');
